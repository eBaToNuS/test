...
setTimeout(() => console.log("Text"), 1000)                         /* Выполнение команды console.log() через 1 сек асинхронно (Параллельно)*/
async function name(params) {                                       /* async указывает, что функция будет выполняться асинхронно*/
  await                                                             /* Приостанавливает асинхронность функции до ее выполнения */
}
try{                                                                /* Пробует выполнить код. При ошибке выполняется код из catch */
  throw new Error('Имя должно быть строкой')                               /* Добавление ошибки. Чато сочитается с if */
}catch(error){
  console.error(`Произошла ошибка: ${error.message}`)                                 /* Объявление ошибки в консоль с сообщением + сообщение из error */
}
fetch                                                                                 /* Встроенный в браузер метод для отправки сетевых запросов к серверу. Чаще http. По умолчанию запрос get */
